...
prepare_DEMOGRsensitive <- function(short_name_scale_str) {
  ...
  # If the raw files exist, process, else, try to get the data from outputs or _targets
  if (length(input_files_sensitive) > 0) {
  ...
  } else if (file.exists(here::here("outputs/data/df_DEMOGR3.rds"))) {
   
    DF_output = read_rds(here::here("outputs/data/df_DEMOGR3.rds"))
     
  } else if (file.exists(here::here("_targets/objects/df_DEMOGR3"))) {
    
    DF_output = read_rds(here::here("_targets/objects/df_DEMOGR3"))
    
  } else {
    
    stop("df_DEMOGR3 data and the sensitive RAW files associated are not available.")
