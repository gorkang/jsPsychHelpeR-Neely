...
`r nrow(DF_joined %>% drop_na(COVIDCONTROL_01_DIR))` participants completed the full protocol. After filtering by exclusion criteria, we can match `r TABLE_matching$MATCHED`. Still  `r TABLE_matching$MISSING` out of `r TABLE_matching$NEEDED` to go  (`r round(TABLE_matching$MISSING / TABLE_matching$NEEDED, 2) * 100`%).  
