...
author: "Neely-Prado, Navarrete, van Elk, Hola, Huepe"
date: "12-07-2021"
...
descriptives = psych::describe(matched %>% select(Religiosity, `Social-Adaptation`, `Self-Esteem`)) 
descriptives = descriptives %>% as.data.frame() %>% select(mean, sd, min, max, skew, kurtosis)
#rownames(descriptives) = c("Religiosity", "Social Adaptation", "`Self-Esteem`", "Age")
descriptives %>% kable()
```
...
The following graphs show the distributions of each variable per group.

```{r cars}
matched_long = matched %>% select(Religiosity, `Social-Adaptation`, `Self-Esteem`, group) %>% 
...
# Alternative version
ggplot(matched_long, aes(values, group, fill = group)) + 
  ggridges::geom_density_ridges(stat = "binline", bins = 30, draw_baseline = FALSE) +
  ggridges::geom_density_ridges(jittered_points = FALSE, alpha = 0.6) +
  scale_fill_manual(values=c("#69b3a2", "#404080")) +
  theme_minimal() +
...
```{r}
apaTables::apa.cor.table(matched %>% select(Religiosity, `Social-Adaptation`, `Self-Esteem`), filename = here::here("outputs/correlation_table.doc"), table.number = 2,show.conf.interval = F)
CORREL
...
x <- corx(matched %>% select(Religiosity, `Social-Adaptation`, `Self-Esteem`),
          triangle = "lower",
...
We explore if there are mean differences between groups for each independent variable.
...
If we consider moderation, there is still no effect.
...
Just for the sake of exploration, we will take a deeper look on the significant relationship between self-esteem and social adaptation. Also, because some studies suggest the relationship between religiosity and well-being variables may depend on how religiosity is measured, we will also explore if different measures of religiosity do predict self-esteem and/or social adaptation.

### Self-Esteem

From the output below we can say that self-esteem does predict social adaptation.
...
### Religiosity and Social Adaptation in non-vulnerable contexts
...
```{r}
#write_csv(matched, "/Users/aneel/Documents/matched.csv")
```
