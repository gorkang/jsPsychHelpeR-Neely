...
data_new = read_csv(here::here("outputs/data/DF_joined.csv")) %>% filter(id %in% id_match)
...
descriptives = describe.by(matched %>% select(Religiosidad, SASS_Total, Autoestima)) 
...
```{r}
library(apaTables)
apa.cor.table(matched %>% select(Religiosidad, SASS_Total, Autoestima), filename = here::here("outputs/correlation_table.doc"), table.number = 2,show.conf.interval = F)
```
